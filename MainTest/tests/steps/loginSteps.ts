...
When('user enters valid {string} and {string}', async function (this: CustomWorld, username: string, password: string) {
    const lognPage = new LoginPage(this.page);
...
Then('user should be redirected to the dashboard', async function (this: CustomWorld) {
  ...
  if (actualUrl !== expectedUrl) {
    throw new Error(`Expected to be on ${expectedUrl} but was on ${actualUrl}`);
  }
});
